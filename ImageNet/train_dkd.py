...
def main_worker(gpu, ngpus_per_node, args, teacher, T_EMB):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.rank % ngpus_per_node == 0:
            ...
            summary_writer.add_scalar('dkd_loss', div_loss, epoch)
...
def train(model, teacher, T_EMB, train_loader, optimizer, criterion, kd_loss, nd_loss, ngpus_per_node, args, epoch):
    ...
    for step, (images, labels) in enumerate(train_loader):
        ...
        s2 = ' - {:.2f}ms/step - nd_loss: {:.3f} - dkd_loss: {:.3f} - cls_loss: {:.3f} - train_loss: {:.3f} - train_acc_top1: {:.3f} - train_acc_top5: {:.3f}'.format(
...
if __name__ == "__main__":
    ...
    # class-mean
    with open("./ckpt/teacher/resnet152/center_emb_train.json", 'r') as f:
        T_EMB = json.load(f)
    f.close()
